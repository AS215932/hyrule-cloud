...
The API server coordinates: XCP-NG XAPI (clone template, set CPU/RAM/disk, inject cloud-init, start VM), DNS (create AAAA record under deploy.hyrule.cloud), and optionally Openprovider (register custom domain).
...
- **Auto subdomains for short-lived deployments.** `<sha256prefix>.deploy.hyrule.cloud` with AAAA pointing to VM IPv6. Custom domains via Openprovider for persistent deployments.
...
- OpenClaw skill (SKILL.md + thin Python client wrapping the API)
- MCP server (for Claude/Cursor direct consumption)
