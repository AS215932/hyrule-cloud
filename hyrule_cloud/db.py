...
class ApiKeyRow(Base):
    """Scoped API key for programmatic VM management (Block D).
    ...
    Scopes are an explicit JSONB list of `ApiKeyScope` values. The middleware
    enforces them on every API-key-authed request; sessions are unrestricted
    (a session = full account access). API keys CANNOT be used for password
    changes, recovery rotation, or account deletion — those are browser-only.
    """

    __tablename__ = "api_keys"

    # uuid4 (str so SQLite test path doesn't need a custom type adapter)
    ...
    # Stored as a JSON array of scope strings. Defaults applied at create-time
    # in api/auth.py — never wildcard, never silently expanded.
