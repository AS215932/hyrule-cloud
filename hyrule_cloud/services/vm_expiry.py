...
def build_vm_expiry(
    ...
    *, now: datetime | None = None, deletion_started_at: datetime | None = None,
    retention_state: str | None = None, retained_until: datetime | None = None,
) -> VMExpiryInfo:
    ...
    if retention_state is not None:
        state, message = {
            "prepared": (VMExpiryState.RETAINING, "VM retention is being verified. Contact support for recovery."),
            "retained": (VMExpiryState.RETAINED, "The VM is stopped and retained for recovery. Contact support to restore it."),
            "restoring": (VMExpiryState.RESTORING, "VM recovery is in progress. Contact support for updates."),
        }.get(retention_state, (VMExpiryState.RETAINING, "VM recovery needs verification. Contact support."))
        return VMExpiryInfo(state=state, observed_at=observed, message=message,
                            retained_until=_utc(retained_until) if retained_until is not None else None)
