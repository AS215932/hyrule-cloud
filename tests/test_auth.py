...
async def test_register_without_api_key_omits_key_fields(auth_state, client):
    """Backward compat: callers that don't ask for a key don't see one."""
    ...
    assert body.get("api_key_scopes") is None


@pytest.mark.asyncio
async def test_register_with_api_key_returns_usable_bearer(auth_state, client):
    """Block D agent bootstrap: with_api_key=True returns a cleartext key that
    immediately authenticates against /v1/me with DEFAULT_API_KEY_SCOPES."""
    from hyrule_cloud.models import DEFAULT_API_KEY_SCOPES
    ...
    assert set(body["api_key_scopes"]) == {s.value for s in DEFAULT_API_KEY_SCOPES}

    # Drop the session cookie; the bearer alone must authenticate.
    bare_client = AsyncClient(transport=ASGITransport(app=app), base_url="http://localhost")
    try:
...
@pytest.mark.asyncio
async def test_register_with_api_key_default_name_when_omitted(auth_state, client):
    """If api_key_name is omitted, the key is stored as 'agent-bootstrap'.
    DEFAULT_API_KEY_SCOPES intentionally excludes api_keys:read, so we read
    the row directly via the DB rather than the /me/api-keys endpoint.
    """
    ...
    res = await client.post(
        "/v1/auth/register",
        json={"password": "correct horse battery staple", "with_api_key": True},
    )
    assert res.status_code == 200
    ...
    async with auth_state.orchestrator.db() as db:
        rows = (await db.execute(
            select(ApiKeyRow).where(ApiKeyRow.account_id == account_id)
        )).scalars().all()
    assert len(rows) == 1
    assert rows[0].name == "agent-bootstrap"
...
async def test_account_owner_can_manage_their_vm(auth_state, client):
    ...
    assert res.status_code == 200
    assert res.json()["owner_wallet"] == "0xPayer"
...
async def test_cross_account_access_returns_404_not_403(auth_state, client):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://localhost") as cb:
        ...
        # All paths must return 404 (NOT 403 — don't leak existence)
        for path in (f"/v1/vm/{vm_id}/status", f"/v1/vm/{vm_id}", f"/v1/vm/{vm_id}/logs"):
...
async def test_account_delete_detach_returns_fresh_tokens(auth_state, client):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://localhost") as cx:
        ...
        assert res.status_code == 200
        assert res.json()["has_anon_management_token"] is True
