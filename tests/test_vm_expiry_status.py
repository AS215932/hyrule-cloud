...
@pytest.mark.asyncio
@pytest.mark.parametrize('retention_state,expected', [('prepared', 'retaining'), ('retained', 'retained'),
                                                     ('restoring', 'restoring'), (None, 'deleting')])
@pytest.mark.parametrize('deadline_days', [-1, 30])
async def test_customer_status_distinguishes_retention_from_deletion(override_state, retention_state, expected, deadline_days):
    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine

    from hyrule_cloud.db import Base, VMRetentionRow
    ...
    row.deletion_started_at = datetime.now(UTC)
    row.metadata_ = {'launch_proof': {'customer_message': 'Your VM is ready.'}}
    override_state.orchestrator.get_vm = AsyncMock(return_value=row)
    override_state.orchestrator.db = factory
    ...
    try:
        ...
        if retention_state is not None:
            async with factory.begin() as session:
                session.add(VMRetentionRow(vm_id=row.vm_id, source_vm_uuid='private-guest',
                                          owner_wallet='fixture', state=retention_state, retain_until=deadline,
                                          manifest={'private': 'secret-manifest'}, restore_config={'private': 'secret-config'}))
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as client:
            public = await client.get('/v1/vm/vm_test123/status')
            owner = await client.get('/v1/vm/vm_test123', headers={'Authorization': 'Bearer ' + _TEST_TOKEN})
            assert public.status_code == owner.status_code == 200
            for response in (public, owner):
                expiry = response.json()['expiry']
                assert expiry['state'] == expected
                assert expiry['deletion_eligible'] is (retention_state is None)
                assert expiry['grace_ends_at'] is None
                if retention_state is not None:
                    assert datetime.fromisoformat(expiry['retained_until']) == deadline
                else:
                    assert expiry['retained_until'] is None
                assert 'secret-' not in response.text and 'private-guest' not in response.text
            assert 'ready' not in public.json()['customer_message'].lower()
            assert 'ssh' not in public.json() and owner.json()['ssh'] is None
    finally:
